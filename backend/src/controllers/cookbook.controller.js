...
//menampilkan resep berdasarkan nama
async function getCookBookRecipesById(req, res) {
  try {
...
//menambah resep baru
async function addCookBookRecipes(req, res) {
  try {
    const body = req.body;
    ...
    if (!body.category_id) {
      return res.status(400).json(responseError("Category ID is required"));
    }

    const data = await models.insert(body.title, body.ingredients, body.instruction, body.caption, body.category_id);
...
async function editCookBookRecipesById(req, res) {
  try {
    const id = req.params["id"];
    const body = req.body;
    if (!id) return res.status(400).json(responseError("ID is required"));
    if (!body.title && !body.ingredients && !body.instruction && !body.caption && !body.category_id) {
...
async function deleteCookBookRecipesById(req, res) {
  try {
    const id = req.params["id"];
...
module.exports = {
  getCookBookRecipes,
  getCookBookRecipesById,
  addCookBookRecipes,
  editCookBookRecipesById,
  deleteCookBookRecipesById,
};
